...
import { Clock, Users, MessageSquare, Share2, ArrowLeft, ThumbsUp, ThumbsDown } from "lucide-react"
import { getDebateDetails, getDebateArguments, getDebateParticipants } from "@/app/actions"
...
export default function PublicDebatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const supportArguments = debateArguments.filter((arg) => arg.side === "support")
  const opposeArguments = debateArguments.filter((arg) => arg.side === "oppose")
      ...
      <div className="flex flex-1 justify-center px-4 py-5 lg:px-40">
        ...
        >
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h1 className="text-2xl font-bold mb-2 text-foreground">{debate.title}</h1>
                      <p className="text-muted-foreground leading-relaxed">{debate.description}</p>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-border">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground">{formatTimeRemaining(debate.endsAt)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm text-muted-foreground">{debate.supportVotes + debate.opposeVotes} participants</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <ThumbsUp className="h-4 w-4 text-green-500" />
                      <span className="text-sm text-muted-foreground">{debate.supportVotes} support</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <ThumbsDown className="h-4 w-4 text-red-500" />
                      <span className="text-sm text-muted-foreground">{debate.opposeVotes} oppose</span>
                    </div>
                  </div>
                </div>
              </CardContent>
                ...
                <CardContent className="space-y-4">
                  {supportArguments.length > 0 ? (
                    supportArguments.map((argument) => (
                      <div key={argument.id} className="border-b border-border pb-4 last:border-b-0">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="font-medium text-foreground">{argument.username}</span>
                    ...
                    <p className="text-muted-foreground text-center py-4">No support arguments yet.</p>
                  )}
                </CardContent>
              </Card>
            </motion.div>
              ...
              <Card className="bg-card border-border">
                ...
                <CardContent className="space-y-4">
                  {opposeArguments.length > 0 ? (
                    ...
                    <p className="text-muted-foreground text-center py-4">No oppose arguments yet.</p>
                  )}
                </CardContent>
              </Card>
            </motion.div>
          ...
          >
            <Card className="bg-card border-border">
              <CardContent className="p-6">
                <h3 className="text-lg font-semibold mb-2 text-foreground">Want to participate?</h3>
                <p className="text-muted-foreground mb-4">
                  Join the debate and share your perspective on this important topic.
                </p>
