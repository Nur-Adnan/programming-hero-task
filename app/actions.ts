...
export async function createDebate(debateData: {
  ...
  try {
    ...
    const endsAt = new Date(
      now.getTime() + debateData.duration * 60 * 60 * 1000
    );
  ...
  try {
    ...
    const argumentsWithVotes = await Promise.all(
      debateArguments.map(async (argument) => {
        ...
        const user = await models.UserModel.findById(argument.userId).select(
          "username"
        );
        const currentUsername =
          user?.username || argument.username || "Unknown User";
...
export async function postArgument(
  ...
  try {
    ...
    const detectedToxicWords = bannedWords.filter((word) =>
    ...
    if (detectedToxicWords.length > 0) {
      return {
        success: false,
        argument: null,
        error: `Inappropriate language detected: ${detectedToxicWords.join(
          ", "
        )}. Please use respectful language to maintain a healthy debate environment.`,
      };
    }
    ...
    if (!participant) {
      return {
        success: false,
        argument: null,
        error: "You must join the debate before posting arguments",
      };
    }
    ...
    if (!participant.hasPostedFirstArgument) {
      ...
      if (timeDiff > fiveMinutesInMs) {
        return {
          ...
          argument: null,
          error:
            "Reply timer expired. You must post your first argument within 5 minutes of joining the debate.",
        };
      }
    }
...
export async function deleteArgument(argumentId: string) {
  try {
    ...
    const deletedArgument = await models.ArgumentModel.findByIdAndDelete(
      argumentId
    );
...
export async function getDebateArgumentsWithUserVotes(
  debateId: string,
  userId?: string
) {
  try {
...
export async function getAllDebates() {
  try {
    ...
    // Find all debates
    const debates = await models.DebateModel.find({}).sort({ createdAt: -1 }); // Most recent first
...
export async function getLiveDebates() {
  try {
    ...
    return {
      success: false,
      debates: [],
      error: "Failed to fetch live debates",
    };
  }
}
...
export async function getLeaderboardData(
  ...
  try {
    ...
    const users = await models.UserModel.find({ _id: { $in: userIds } }).select(
      "username"
    );
  ...
  try {
    ...
    if (!participant) {
      return {
        success: false,
        canPost: false,
        error: "User is not a participant in this debate",
      };
    }
    ...
    return {
      success: false,
      canPost: false,
      error: "Failed to check user posting status",
    };
  }
}
...
export async function generateDebateSummary(debateId: string) {
  try {
    ...
    const supportArguments = argumentsWithVotes.filter(
      (arg) => arg.side === "support"
    );
    const opposeArguments = argumentsWithVotes.filter(
      (arg) => arg.side === "oppose"
    );
    ...
    const totalVotes = argumentsWithVotes.reduce(
      (sum, arg) => sum + Math.abs(arg.voteCount),
      0
    );
    ...
    if (debate.winner === "tie") {
      ...
      const winningSide =
        debate.winner === "support" ? "support" : "opposition";
