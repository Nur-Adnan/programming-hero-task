...
import { Zap, Plus, Search, Trophy, Settings, LogOut, Menu, X, ChevronDown } from "lucide-react"
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-0 bg-background/95 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80 shadow-lg shadow-black/10 dark:bg-transparent dark:shadow-none">
      <div className="container flex h-16 items-center justify-between px-4 md:px-6">
        {/* Logo/Brand */}
        <Link href="/" className="flex items-center gap-3 font-bold text-foreground dark:text-white">
        ...
        <div className="flex items-center gap-3">
          ...
          {currentUser ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56 bg-card/95 backdrop-blur-xl border-0 text-card-foreground shadow-2xl shadow-black/30 dark:bg-black/80 dark:border dark:border-white/20 dark:text-white dark:shadow-black/50" align="end" forceMount>
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    <p className="text-sm font-semibold leading-none text-card-foreground dark:text-white">{currentUser?.username || "Guest"}</p>
                    <p className="text-xs leading-none text-muted-foreground dark:text-white/60">{currentUser?.email || "N/A"}</p>
                  </div>
                </DropdownMenuLabel>
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/signup">
                <Button 
                  className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white shadow-lg hover:shadow-xl transition-all duration-300 rounded-full px-6 py-2 hover:scale-105 hover:animate-glow-pulse"
                >
                  Get Started
                </Button>
              </Link>
